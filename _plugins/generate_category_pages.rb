...
module Jekyll
    ...
    class CategoryPages < Generator
        ...
        def paginate(site, page)
            ...
            pages = CategoryPager.calculate_pages(category_posts, 1000) #site.config['paginate'].to_i)
    ...
    class CategorySubPage < Page
        ...
        def initialize(site, base, category, layout)
            ...
            self.process(@name)
            self.read_yaml(File.join(base, '_layouts'), layout || 'category.yml')
    ...
    #module Filters
        ...
        #def pager_links(pager)
                ...
                #html = '<div class="pager clearfix">'
                        ...
                        #html << "<a href=\"/#{pager['category']}/\">«</a>"
                    #else
                        #html << "<a href=\"/#{pager['category']}/page#{pager['previous_page']}\">«</a>"
                    #end
                #else
                    #html << "«"
                #end
                #html << "</strong>&nbsp;This is the page <strong>#{pager['page']}</strong> out of <strong>{#{pager['total_pages']}</strong>.<strong>&nbsp;"

                #if pager['next_page'] 
                    #html << "<a href=\"/#{pager['category']}/page#{pager['next_page']}\">»</a>"
                #else
                    #html << "»"
                #end
