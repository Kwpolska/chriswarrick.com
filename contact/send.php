...
die ("failed.<br><div class=\"error\"><strong>ERROR:</strong> $contents</div>");
}
echo "<strong>Checking CAPTCHA...</strong>";
if ($captcha != md5($_POST['captcha'])) showError("This is not the right CAPTCHA phrase."); //md5 = idiot-proof
echo " bypassed.<br><strong>Checking the fields...</strong>";
if ($_POST['name'] == '' || $_POST['mail'] == '' || $_POST['subject'] == '' || $_POST['message'] == '') showError('You haven\'t filled all the fields.');
...
$message = $_POST['message'].PHP_EOL.'---'.PHP_EOL.'      Sent by: '.$_SERVER['REMOTE_ADDR'].PHP_EOL.'      Sent at: '.date('l, F jS, Y g:i:s A').PHP_EOL.'Dump of $_POST: '.PHP_EOL.$dump; //useful^H^H^Hless data also included
